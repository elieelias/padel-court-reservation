...
export function FriendManager() {
  ...
  return (
    <section className="panel friends-card">
        ...
        <div className="friend-results">
          {results.map((result) => (
            <article className="friend-row" key={result.player_id}>
              <span className="friend-avatar">{result.username.slice(0, 1).toUpperCase()}</span>
              <div><strong>@{result.username}</strong><small>{result.relationship_direction === "friends" ? t("friends.alreadyFriends") : result.relationship_direction === "outgoing" ? t("friends.pending") : result.relationship_direction === "incoming" ? t("friends.waitingForYou") : t("friends.player")}</small></div>
      ...
      {incoming.length > 0 && <div className="friend-group"><h3>{t("friends.requests")}</h3>{incoming.map((item) => <article className="friend-row" key={item.friendship_id}><span className="friend-avatar">{item.username.slice(0, 1).toUpperCase()}</span><div><strong>@{item.username}</strong><small>{t("friends.wantsToConnect")}</small></div><div className="friend-row__actions"><button aria-label={t("friends.accept")} className="friend-icon-button is-accept" disabled={workingId === item.friendship_id} onClick={() => void respond(item.friendship_id, true)} type="button"><Check aria-hidden="true" size={18} /></button><button aria-label={t("friends.decline")} className="friend-icon-button" disabled={workingId === item.friendship_id} onClick={() => void respond(item.friendship_id, false)} type="button"><X aria-hidden="true" size={18} /></button></div></article>)}</div>}

      <div className="friend-group"><h3>{t("friends.yourFriends", { count: accepted.length })}</h3>{loading ? <p className="friends-empty">{t("common.loading")}</p> : accepted.length ? accepted.map((item) => <article className="friend-row" key={item.friendship_id}><span className="friend-avatar">{item.username.slice(0, 1).toUpperCase()}</span><div><strong>@{item.username}</strong><small>{t("friends.readyToBook")}</small></div></article>) : <p className="friends-empty">{t("friends.noFriends")}</p>}</div>
